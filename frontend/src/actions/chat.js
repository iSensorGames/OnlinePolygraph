...
export const CHAT_SET_GAME_READYTOPLAYOPPONENT =
  "chat/CHAT_SET_GAME_READYTOPLAYOPPONENT";
...
export const setGame = params => {
  return (dispatch, getState, { socket }) => {
    ...
    socket.setGame(room.id, params).catch(error => {
      dispatch({
...
export const setGameReadyToPlay = () => {
  return async (dispatch, getState, { socket }) => {
    const state = getState();
    const room = chatSelectors.getRoom(state);
    await socket.setGameReadyToPlay(room.id);
  };
};
